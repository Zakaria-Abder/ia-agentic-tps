...
def echo_node(state: ChatState):
    return {
        "messages": [{"role": "ai", "content": f"Step {state['steps']}: got your message."}],"steps": state["steps"] + 1,
    }

def echo_node_1(state: ChatState):
    return {
        "messages": [{"role": "ai", "content": f"Step {state['steps']}: got your message 1."}],"steps": state["steps"] + 1,
    }
